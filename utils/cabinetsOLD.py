...
def process_cabinet(cab_path):
    ...
    if not doc_dir.exists():
        Logger.error(f"Папка Документация не найдена: {doc_dir}")
        return
    
    dwg_files = list(doc_dir.glob("*.dwg"))
    if not dwg_files:
        Logger.error(f"DWG файлы не найдены в: {doc_dir}")
        return
    
    Logger.info(f"Найден DWG: {dwg_files[0].name}")
...
def process_cabinets():
    ...
    for cab in target:
        base_path = path / cab
        Logger.info(f"Обработка: {cab}")
        process_cabinet(base_path)

    Logger.success("Пакетная обработка РЭ ШЭТ завершена...")